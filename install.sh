...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for coinbase/rosetta-metrix 
...
execute() {
  ...
  docker tag "rosetta-metrix:${TAG}" "rosetta-metrix:latest"
  log_info "loaded rosetta-metrix:${TAG} and tagged as rosetta-metrix:latest"
  rm -rf "${tmpdir}"
